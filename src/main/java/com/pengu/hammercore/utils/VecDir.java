...
public class VecDir
{
	...
	public <T extends Entity> List<? extends T> getEntitiesWithinDir(World world, Class<? extends T> ent)
	{
		...
		AxisAlignedBB aabb = new AxisAlignedBB(start.xCoord, start.yCoord, start.zCoord, end.xCoord, end.yCoord, end.zCoord).expand(.5, .5, .5);
	...
	public <T extends Entity> T getClosestEntityWithinDir(World world, Class<? extends T> ent)
	{
		...
		for(T t : ts)
			if((curr = t.getDistanceSq(start.xCoord, start.yCoord, start.zCoord)) < min && (last = t) != null)
				min = curr;
