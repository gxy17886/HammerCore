...
public class SpriteSheetManager {
	...
	public static class SpriteSheet implements IIconRegister
	{
		...
		public TextureAtlasSprite getSprite(int index) {
			...
			if (i == null) {
				throw new IllegalArgumentException("Sprite at index: " + index
						+ " from texture file " + resource
						+ " was not preloaded.");
			}
			return i;
		}
		...
		public TextureDataHolder createSprite(int spriteIndex) {
			...
			TextureDataHolder sprite = new TextureDataHolder(spriteWidth,
					spriteHeight);
	...
	public static SpriteSheet getSheet(int tilesX, int tilesY,
			ResourceLocation resource) {
		SpriteSheet sheet = spriteSheets.get(resource.toString());
		if (sheet == null) {
			spriteSheets.put(resource.toString(), sheet = new SpriteSheet(
					tilesX, tilesY, resource));
		}
		return sheet;
	}
}