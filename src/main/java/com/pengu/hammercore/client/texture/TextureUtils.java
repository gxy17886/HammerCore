...
public class TextureUtils
{
	...
	public static InputStream getTextureResource(ResourceLocation textureFile)
			throws IOException {
		return Minecraft.getMinecraft().getResourceManager()
				.getResource(textureFile).getInputStream();
	}
	...
	public static void copySubImg(int[] fromTex, int fromWidth, int fromX,
			int fromY, int width, int height, int[] toTex, int toWidth,
			int toX, int toY) {
	...
	public static TextureAtlasSprite getBlankIcon(int size,
			TextureMap textureMap) {
		String s = "blank_" + size;
		TextureAtlasSprite icon = textureMap.getTextureExtry(s);
		if (icon == null) {
			icon = new TextureSpriteCustom(s).blank(size);
	...
	public static TextureSpriteCustom getTextureSpriteCustom(
			TextureMap textureMap, String name) {
		if (textureMap.getTextureExtry(name) != null) {
			throw new IllegalStateException("Texture: " + name
					+ " is already registered");
		}

		TextureSpriteCustom icon = new TextureSpriteCustom(name);
		textureMap.setTextureEntry(icon);
		return icon;
	}

	public static void prepareTexture(int target, int texture,
			int min_mag_filter, int wrap) {
		GlStateManager.glTexParameteri(target, GL11.GL_TEXTURE_MIN_FILTER,
				min_mag_filter);
		GlStateManager.glTexParameteri(target, GL11.GL_TEXTURE_MAG_FILTER,
				min_mag_filter);
		...
		case GL12.GL_TEXTURE_3D:
			GlStateManager
					.glTexParameteri(target, GL12.GL_TEXTURE_WRAP_R, wrap);
		case GL11.GL_TEXTURE_2D:
			GlStateManager
					.glTexParameteri(target, GL11.GL_TEXTURE_WRAP_T, wrap);
		case GL11.GL_TEXTURE_1D:
			GlStateManager
					.glTexParameteri(target, GL11.GL_TEXTURE_WRAP_S, wrap);
		}
	}
	...
	public static TextureAtlasSprite getBlockTexture(ResourceLocation location) {
		return getTexture(new ResourceLocation(location.getResourceDomain(),
				"blocks/" + location.getResourcePath()));
	}
	...
	public static TextureAtlasSprite getItemTexture(ResourceLocation location) {
		return getTexture(new ResourceLocation(location.getResourceDomain(),
				"items/" + location.getResourcePath()));
	}
	...
	public static TextureAtlasSprite[] getIconsForBlock(IBlockState state,
			int side) {
	...
	public static TextureAtlasSprite[] getIconsForBlock(IBlockState state,
		...
		if (model != null) {
			...
			if (quads != null && quads.size() > 0) {
				TextureAtlasSprite[] sprites = new TextureAtlasSprite[quads
						.size()];
