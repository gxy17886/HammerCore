					...
					if(j < s.length() - AUTHOR_DNAME.length() && s.substring(j, j + AUTHOR_DNAME.length()).equals(AUTHOR_DNAME))
					{
						...
						if(j1 < 200 || c.getChatOpen())
						{
							...
							if((20 * l1) / 255 > 3)
							{
								...
								drawTextGlowingAuraTransparent(Minecraft.getMinecraft().fontRenderer, AUTHOR_USERNAME, chatX + 2 + Minecraft.getMinecraft().fontRenderer.getStringWidth(before), chatY - (Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT) * i, l1);
								GL11.glTranslated(-.25, 0, 0);
