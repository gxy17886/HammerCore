...
public class SnowfallHooks
{
	private static void meltSnow(World world, BlockPos pos, IBlockState state, Random rand)
	{
		...
		{
			if(world.getLightFor(EnumSkyBlock.BLOCK, pos) > 11 || (world.getBiome(pos).getFloatTemperature(pos) >= .15F && HammerCoreConfigs.snowfall_meltSnow))
		...
		if(world.getLightFor(EnumSkyBlock.BLOCK, pos) > 11 || (world.getBiome(pos).getFloatTemperature(pos) >= .15F && HammerCoreConfigs.snowfall_meltSnow))
		{
			while(world.getBlockState(pos = pos.up()).getBlock() == Blocks.SNOW_LAYER)
				;
			pos = pos.down();
			
			meta = Blocks.SNOW_LAYER.getMetaFromState(world.getBlockState(pos)) & 0x7;
			try
			{
				if(meta < 1)
					world.setBlockToAir(pos);
			...
			while(al.getPos().getY() > 0 && (al.getBlock() == Blocks.AIR || al.getBlock() == Blocks.SNOW_LAYER))
			{
				if(al.getBlock() == Blocks.SNOW_LAYER)
				{
					...
					if(fm - lm > 1 && !down)
					{
						...
						List<Entity> ents = world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(l.getPos()));
						for(Entity e : ents)
					...
					} else if(lm > 0 && down)
					{
						...
						List<Entity> ents = world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(al.getPos()));
						for(Entity e : ents)
							e.move(MoverType.PISTON, 0, .125, 0);
