...
public class GuiShareToLanImproved extends GuiShareToLan
{
	...
	protected void actionPerformed(GuiButton b) throws IOException
	{
		...
		if(b.id == 101)
		{
			...
			mc.player.sendMessage(new TextComponentString("   PvP: " + (LanUtil.pvp ? "on" : "off")));
			mc.player.sendMessage(new TextComponentString("   Online: " + (LanUtil.online ? "on" : "off")));
			mc.player.sendMessage(new TextComponentString("   Max Players: " + LanUtil.maxPlayers));
	...
	public void drawScreen(int p_drawScreen_1_, int p_drawScreen_2_, float p_drawScreen_3_)
	{
		...
		fontRenderer.drawString("Open on port: (1000 - 65535 or 0)", width / 2 - 155, 135, 16777215, false);
		fontRenderer.drawString("Max. Players: (1 - 128)", width / 2 - 155, 170, 16777215, false);
	}
