...
public class RecipeRenderer
{
	...
	{
		if(recipe instanceof ShapedOreRecipe) render((ShapedOreRecipe) recipe);
		if(recipe instanceof ShapelessOreRecipe) render((ShapelessOreRecipe) recipe);
		if(recipe instanceof ShapedRecipes) render((ShapedRecipes) recipe);
		if(recipe instanceof ShapelessRecipes) render((ShapelessRecipes) recipe);
	}
	...
	public static void renderPattern(Object[] recipe)
	{
		...
		if(recipe.length <= 4) for(int i = 0; i < Math.min(recipe.length, 4); ++i)
		{
			int x = i % 2;
			int y = i / 2;
			Object o = recipe[i];
			...
			if(o instanceof ItemStack) Minecraft.getMinecraft().getRenderItem().renderItemAndEffectIntoGUI((ItemStack) o, x * 18, y * 18);
		...
		if(recipe.length >= 9) for(int i = 0; i < Math.min(recipe.length, 9); ++i)
		{
			int x = i % 3;
			int y = i / 3;
			Object o = recipe[i];
