...
public class GuiShareToLanImproved extends GuiShareToLan
{
	...
	public void initGui()
	{
		...
		txtPort = new GuiTextField(0, fontRendererObj, width / 2 - 155, 145, 150, 20);
		txtPort.setVisible(true);
		
		txtMaxPlayers = new GuiTextField(0, fontRendererObj, width / 2 - 155, 180, 150, 20);
	...
	protected void actionPerformed(GuiButton b) throws IOException
	{
		...
		if(b.id == 101)
		{
			...
			mc.thePlayer.addChatMessage(new TextComponentString("Server properties:"));
			mc.thePlayer.addChatMessage(new TextComponentString("   PvP: " + (LanUtil.pvp ? "on" : "off")));
			mc.thePlayer.addChatMessage(new TextComponentString("   Online: " + (LanUtil.online ? "on" : "off")));
			mc.thePlayer.addChatMessage(new TextComponentString("   Max Players: " + LanUtil.maxPlayers));
	...
	public void drawScreen(int p_drawScreen_1_, int p_drawScreen_2_, float p_drawScreen_3_)
	{
		...
		fontRendererObj.drawString("Open on port: (1000 - 65535 or 0)", width / 2 - 155, 135, 16777215, false);
		fontRendererObj.drawString("Max. Players: (1 - 128)", width / 2 - 155, 170, 16777215, false);
	}
	...
	protected void keyTyped(char par1, int par2) throws IOException
	{
		...
		b.enabled = (LanUtil.port >= 1000 && LanUtil.port <= 65535 || LanUtil.port == 0) && LanUtil.maxPlayers > 0 && LanUtil.maxPlayers <= 128;
	}
}