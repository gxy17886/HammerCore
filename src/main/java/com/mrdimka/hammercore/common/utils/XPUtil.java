...
public class XPUtil
{
	...
	{
		int level = 0;
		if(value >= getXPValueFromLevel(30)) level = (int) (.07142857142857142 * (Math.sqrt(56D * value - 32511D) + 303D));
		else if (value >= getXPValueFromLevel(15)) level = (int) (.16666666666666666 * (Math.sqrt(24D * value - 5159D) + 59D));
		else level = (int) (value / 17D);
		return level;
	}
	...
	{
		int val = 0;
		if(xpLevel >= 30) val = (int) (3.5 * Math.pow(xpLevel, 2D) - 151.5 * xpLevel + 2220D);
		else if (xpLevel >= 15) val = (int) (1.5D * Math.pow(xpLevel, 2D) - 29.5 * xpLevel + 360D);
		else val = 17 * xpLevel;
		return val;
	}
	...
	{
		int val = 0;
		if(xpLevel >= 30) val = 7 * xpLevel - 148;
		else if (xpLevel >= 15) val = 3 * xpLevel - 28;
		else val = 17;
		return val;
	}
}