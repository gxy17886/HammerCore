	...
	public void reload()
	{
		...
		for(Field f : fields) try
		{
			...
			if(type == String.class)
			{
				...
				if(prop != null && flag) f.set(listener, configs.getString(prop.name(), prop.category(), prop.defaultValue(), prop.comment()));
				else if(prop != null) f.set(listener, configs.getString(prop.name(), prop.category(), prop.defaultValue(), prop.comment(), prop.allowedValues()));
			}
			...
			if(type == String[].class)
			{
				...
				boolean flag = prop.allowedValues() == null || prop.allowedValues().length == 0;
				if(prop != null && flag) f.set(listener, configs.getStringList(prop.name(), prop.category(), prop.defaultValue(), prop.comment()));
				else if(prop != null) f.set(listener, configs.getStringList(prop.name(), prop.category(), prop.defaultValue(), prop.comment(), prop.allowedValues()));
			}
			
			if(type == int.class || type == Integer.class)
			{
				ModConfigPropertyInt prop = f.getAnnotation(ModConfigPropertyInt.class);
				if(prop != null) f.set(listener, configs.getInt(prop.name(), prop.category(), prop.defaultValue(), prop.min(), prop.max(), prop.comment()));
			}
			
			if(type == float.class || type == Float.class)
			{
				ModConfigPropertyFloat prop = f.getAnnotation(ModConfigPropertyFloat.class);
				if(prop != null) f.set(listener, configs.getFloat(prop.name(), prop.category(), prop.defaultValue(), prop.min(), prop.max(), prop.comment()));
			}
			
			if(type == boolean.class || type == Boolean.class)
			{
				ModConfigPropertyBool prop = f.getAnnotation(ModConfigPropertyBool.class);
				if(prop != null) f.set(listener, configs.getBoolean(prop.name(), prop.category(), prop.defaultValue(), prop.comment()));
			}
		} catch(Throwable err) {}
