...
public class DebugClassWriter
{
	...
	private static ArrayList<Class<?>> getClasses(Pattern patternPath)
	{
		...
		for(String cl : getResources(patternPath))
		{
			...
			for(int i = 0; i < table.length; ++i) if(table[i] == '/' || table[i] == '\\') table[i] = '.';
			cl = new String(table);
			try { cls.add(Class.forName(cl, false, ClassLoader.getSystemClassLoader())); } catch(Throwable err) {} //Failed to load, skip.
		}
		return cls;
	}
	...
	private static ArrayList<String> getResources(Pattern pattern)
	{
		...
		for(String element : classPathElements) retval.addAll(getResources(element, pattern));
		return retval;
	}
	...
	private static ArrayList<String> getResources(String element, Pattern pattern)
	{
		...
		if(file.isDirectory()) retval.addAll(getResourcesFromDirectory(file, pattern, file.getAbsolutePath() + "/")); else retval.addAll(getResourcesFromJarFile(file, pattern));
		return retval;
	}
	...
	private static ArrayList<String> getResourcesFromJarFile(File file, Pattern pattern)
	{
		ArrayList<String> retval = new ArrayList<String>();
		ZipFile zf;
		try { zf = new ZipFile(file); } catch(ZipException e) { throw new Error(e); } catch(FileNotFoundException e) { return retval; } catch(IOException e) { throw new Error(e); }
		...
		try { zf.close(); } catch(IOException e1) { throw new Error(e1); }
		return retval;
	}
		...
		for(File file : fileList)
		{
			...
			}else
			{
				try
				{
					...
					for(int i = 0; i < cs.length; ++i) if(cs[i] == File.separatorChar) cs[i] = '/';
					fileName = new String(cs);
					boolean accept = pattern.matcher(fileName).matches();
					if(accept) retval.add(fileName.substring(begin.length()));
