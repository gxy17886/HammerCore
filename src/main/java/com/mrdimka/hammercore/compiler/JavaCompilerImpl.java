...
public class JavaCompilerImpl
{
	public static byte[] compile(byte[] source, String srcName)
	{
		...
		if(srcStr.startsWith("package ")) srcPackage = srcStr.substring(8, srcStr.indexOf(";"));
		...
		if(!packagePath.isEmpty()) inf = new File(inf, packagePath);
		File outf = new File(System.getProperty("java.io.tmpdir"), "jdt_out" + System.currentTimeMillis());
		outf.deleteOnExit();
		
		byte[] comp = new byte[0];
		
//		while(srcName.contains(".")) srcName = srcName.substring(srcName.indexOf("."));
		...
		try
		{
			...
			if(!packagePath.isEmpty()) out = new File(new File(outf, packagePath), srcName + ".class");
			else out = new File(outf, srcName + ".class");
			
			if(!out.isFile()) out = new File(outf, packagePath).listFiles()[0];
			...
			try
			{
				...
				int read = 0;
				while((read = fis.read(buf)) > 0) to.write(buf, 0, read);
			}
			catch(Throwable err) {}
			comp = to.toByteArray();
	...
	public static void compileFileIntoJar(String src, File jarFile) throws IOException
	{
		...
		try
		{
			...
			if(tmp.listFiles() != null) for(File f : tmp.listFiles())
			{
				...
				int read = 0;
				while((read = from.read(buf)) > 0) jos.write(buf, 0, read);
