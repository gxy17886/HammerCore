...
public class CommandBuildStructure extends CommandBase
{
	...
	{
		return args.length == 1 ? new ArrayList<>(StructureAPI.GetAllStructures()) : args.length > 1 && args.length < 5 ? getTabCompletionCoordinate(args, 1, targetPos) : new ArrayList<>();
	}
}